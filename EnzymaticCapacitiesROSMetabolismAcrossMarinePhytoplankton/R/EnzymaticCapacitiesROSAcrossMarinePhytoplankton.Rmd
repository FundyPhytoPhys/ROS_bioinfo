---
title: Enzymatic Capacities for Reactive Oxygen Species Production and Scavenging Across Marine Phytoplankton
author:
- Naaman M. Omar:
    institute: mta
- Katherine Fleury:
...
All phytoplankton lineages are photosynthetic and produce multiple ROS as by-products of their metabolism [@vassRoleChargeRecombination2011]. ROS is produced largely in the chloroplast, mitochondria, and peroxisomes, but can also be generated in the plasma membrane, apoplast, endoplasmic reticulum, and cell wall [@apelREACTIVEOXYGENSPECIES2004; @sharmaReactiveOxygenSpecies2012] (Figure 1). Outside the cell multiple reactions generate ROS in seawater, or indeed in seawater-based media [@jeffreymorrisContinuousHydrogenPeroxide2013]. Rainfall on the ocean can temporarily increase extracellular H~2~O~2~ concentrations to greater than 10 times the baseline concentration of surface seawater, which ranges from 114-820 x 10^-7^ mol L^-1^ in the Gulf of Mexico, or  84-206 x 10^-7^ mol L^-1^ in the Atlantic Ocean [@cooperContributionRainwaterVariability1987].
...
EukHyPeDotPlot <- MergedData_na %>%
  filter(HyPe == "Production" | HyPe == "Scavenging", 
         ...
         GeneModels_count != 0) %>%
  ggplot() +
  geom_point(aes(x = EnzymeName, y= reorder(Name, -Radius_um), size = ROSGene_count, color = TaxaPhylum, shape = Ome)) +
