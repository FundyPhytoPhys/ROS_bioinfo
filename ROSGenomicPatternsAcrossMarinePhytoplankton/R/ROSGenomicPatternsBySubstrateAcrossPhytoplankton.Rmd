...
H2O2 production and scavenging are a steady allocation with increasing cell size, relative to the ribosomal housekeeping gene proxy.
But, genes for H2O2 production and scavenging are diluted by increasing total gene counts with increasing cell size, but production declines more than scavenging. XXX
...
When binned across cell sizes, colony forming phytoplankton have a smaller proportion of their total gene content encoding both H~2~O~2~ production (p-value = `r ScientificTen(HyPeProdColonyPlot_pval$p)`), and also H~2~O~2~ scavenging (Fig \@ref(fig:HyPeColonyPlot), p-value = `r ScientificTen(HyPeScavColonyPlot_pval$p)`), than do non-colony forming phytoplankton. The decrease in the fraction of total genes encoding H~2~O~2~ production between non-colony and colony forming phytoplankton is larger (`r signif(HyPeColonyDelta$deltaProduction[[1]]*100, 4)`%), than the decrease in the fraction of total genes encoding H~2~O~2~ scavenging (`r signif(HyPeColonyDelta$deltaScavenging[[1]]*100, 4)`%). In contrast, XXX . H2O2 production and scavenging are a larger allocation in colony formers, relative to ribosomal housekeeping proxies. These contrasting patterns may reflect higher genomic complexity in colony forming strains, which dilutes H~2~O~2~ production and scavenging scavenging genes, even though H~2~O~2~ production and scavenging are enriched relative to a housekeeping proxy. 
...
A binomial model comparing the presence or absence of genes that encode the production of ^•^NO shows no cell size effect (slope = `r ScientificTen(NitOxRadBinom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 2))`, p = `r ScientificTen(NitOxRadBinom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "p.value", 2))`). There is however a cell size effect on presence of ^•^NO producing genes including the influence of Flagella, with larger phytoplankton showing a lower probability for ^•^NO production presence, and flagellated phytoplankton showing a higher probability for presence of production of ^•^NO than their non flagellated counterparts (Figure XXX). In contrast, larger phytoplankton are more likely to have a gene encoding ^•^NO scavenging (slope = `r ScientificTen(NitOxRadBinom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 2))`, p = `r ScientificTen(NitOxRadBinom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "p.value", 2))`). Flagellated phytoplankton have a higher likelihood of presence of ^•^NO scavenging than do their non flagellated phytoplankton XXX.
