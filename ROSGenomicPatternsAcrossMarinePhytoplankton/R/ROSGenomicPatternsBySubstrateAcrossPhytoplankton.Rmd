...
The absence of transcripts encoding SOD from the *Micromonas polaris* transcriptome is likely due to low expression of SOD at the time that the mRNA was harvested for sequence analyses. Given that the O~2~^•−^ is not membrane diffusible, intracellularly produced O~2~^•−^ would have to be scavenged or would react with other parts of the cell, as a result, organisms that do not have the ability to scavenge O~2~^•−^ would have reduced fitness. It would therefore be costly for marine phytoplankton to lose the genomic capacity to scavenge O~2~^•−^. To our knowledge, our results though rare, are the first detection of the occurence of SOO and SOR in marine phytoplankton, and would need confirmation by Multiple Sequence Alignment. Multiple Sequence Alignment would answer whether our results are from errors in annotation from the non-supervised grouping of orthologs (genes) by eggNOG, or may indeed be sporadically present in the Genomes or Transcriptomes of some phytoplankton, perhaps as a result of Horizontal Gene Transfer. Trends may yet appear in genomic allocations to O~2~^•−^ scavenging based on the metallo-types of SOD. For example, in previous pilot runs we found that pico-prasinophytes use Mn-SOD instead of Fe-SOD (Data not presented). 
...
Our results support our hypothesis that differential diffusional exchange across diatoms of different shape does not influence the fraction of total gene content that encodes O~2~^•−^ scavenging enzymes, because O~2~^•−^ diffusion is blocked by the cell membrane irrespective of cell shape (Hypothesis \@ref(hyp:SupOx)).
...
```{r EukHyPeDotPlot, fig.height = 20, fig.width = 10, echo = FALSE}
...
```{r ProkHyPeDotPlot, fig.height = 10, fig.width = 10, echo = FALSE}
...
```{r EukSupOxDotPlot, fig.height = 20, fig.width = 10, echo = FALSE}
...
```{r ProkSupOxDotPlot, fig.height = 10, fig.width = 10, echo = FALSE}
...
```{r EukNitOxDotPlot, fig.height = 20, fig.width = 10, echo = FALSE}
...
```{r ProkNitOxDotPlot, fig.height = 10, fig.width = 10, echo = FALSE}
