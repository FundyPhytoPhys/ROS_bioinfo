...
XXXXMOVE THIS TO A NOTE IN THE RELEVANT FIGURE CAPTION? IT IS A DISTRACTIONXXXXThe absence of transcripts encoding SOD from the *Micromonas polaris* transcriptome is likely due to low expression of SOD at the time that the mRNA was harvested for sequence analyses.XXXX 
...
Including 'Flagella' and 'Colony' as co-variates in the regression results, however, in a slope that is not statistically different from zero (Fig \@ref(fig:SupOxRadPlots), Black line, Slope = `r ScientificTen(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck('flagcol_offset_model_regression', 1, "estimate", 2)/ln_log10_conversion)` ± `r ScientificTen(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck('flagcol_offset_model_regression', 1, "std.error", 2)/ln_log10_conversion)`, p-value = `r ScientificTen(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck('flagcol_offset_model_regression', 1, "p.value", 2))`, pseudo R^2^ = `r 1-(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck('flagcol_offset_model', 1, "deviance")/SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck('flagcol_offset_model', 1, "null.deviance"))`), driven by the influence of 'Flagella' (p-value = `r ScientificTen(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck("flagcol_offset_model_pvalue", 1, tidy, "p.value", 3))`) but not 'Colony' (p-value = `r ScientificTen(SupOxRad %>% filter(SupOx == "Scavenging") %>% pluck("flagcol_offset_model_pvalue", 1, tidy, "p.value", 4))`). Including data from selected prokaryotic phytoplankton did not qualitatively alter these results (Data not shown)XXXXYOU ARE NOT SUPPOSED TO SAY THIS? DO A GLOBAL REPLACE?XXX. O~2~^•−^ metabolism in phytoplankton appears to be mediated by a nearly fixed set of core genes that do not change with increasing gene count, thus the fractional gene allocation to O~2~^•−^ decreases as cell radius, and the co-varying total gene count increases. Therefore, gene dosage does not emerge as a factor in phytoplankton O~2~^•−^ metabolism.
...
XXX When normalized to total ribosomal genes, the proportion of genes encoding both H2o2 production and scavenging is smaller in flagellated organisms than in their non flagellated counterparts.
...
XXX Colony forming organisms have a higher fraction of their housekeeping genes that encode both h2o2 and scavenging.
...
XXXXTYPO HERE???XXX Presence absence = non-flagellated phytoplankton have `r logit2prob(NitOxFlagella %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxFlagella %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide producing gene as opposed to `r logit2prob(NitOxFlagella %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxFlagella %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` in flagellated phytoplankton (p-value = `r ScientificTen(NitOxFlagella %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "p.value", 2))`. Non flagellated phytoplankton also have a `r logit2prob(NitOxFlagella %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxFlagella %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide scavenging gene, as opposed to `r logit2prob(NitOxFlagella %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxFlagella %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` in flagellated phytoplankton (p-value = `r ScientificTen(NitOxFlagella %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "p.value", 2))`.
...
XXXXTYPO HERE?XXX Presence absence = non-colony forming phytoplankton have `r logit2prob(NitOxColony %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxColony %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide producing gene as opposed to `r logit2prob(NitOxColony %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxColony %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` in colony forming phytoplankton (p-value = `r ScientificTen(NitOxColony %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "p.value", 2))`. Non colony forming phytoplankton also have a `r logit2prob(NitOxColony %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxColony %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide scavenging gene, as opposed to `r logit2prob(NitOxColony %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxColony %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` in colony forming phytoplankton (p-value = `r ScientificTen(NitOxColony %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "p.value", 2))`.
...
XXXXTYPO HERE??XXXX Presence absence = centric diatoms have `r logit2prob(NitOxDiatom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxDiatom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide producing gene as opposed to a `r logit2prob(NitOxDiatom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxDiatom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "estimate", 1))` probability in pennate diatoms (p-value = `r ScientificTen(NitOxDiatom %>% filter(NitOx == "Production") %>% pluck("param_b_offset", 1, "p.value", 2))`. Centric diatoms also have a `r logit2prob(NitOxDiatom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*0 + NitOxDiatom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` probability of having a nitric oxide scavenging gene, as opposed to a `r logit2prob(NitOxDiatom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate",2)*1 + NitOxDiatom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "estimate", 1))` probability in colony forming phytoplankton (p-value = `r ScientificTen(NitOxDiatom %>% filter(NitOx == "Scavenging") %>% pluck("param_b_offset", 1, "p.value", 2))`.
