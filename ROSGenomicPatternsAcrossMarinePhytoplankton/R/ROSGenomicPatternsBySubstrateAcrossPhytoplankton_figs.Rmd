...
ggsave(paste("GeneModelsRadPlot",".png",sep = ""), plot = GeneModelsRadPlot, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("RibosomeRadPlot",".png",sep = ""), plot = RibosomeRadPlot, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
ggsave(paste("FigureS4",".png",sep = ""), plot = RibosomeRadPlot, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("FigureS8",".png",sep = ""), plot = SupOxColonyRiboPlot, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
```
...
HyPeRadRiboPlots <- rbind(HyPeRadRibo, HyPeRadProkRibo) %>%
  ...
  unnest(c(offset_model_pvalue), names_sep = "_") %>%
  unnest(c(taxa_offset_model_pvalue), names_sep = "_") %>%
  ...
  geom_text(aes(x = 1.1, y = -1.5, label = paste("p-value =", (signif(`taxa_offset_model_regression_p.value_.$log_Radius_um`, digits = 2)))), color = "darkgreen", check_overlap = TRUE) +
  # geom_point(data = MergedData %>% filter(Taxa == "Prokaryote", HyPe == "Production" | HyPe == "Scavenging"), aes(x = log_Radius_um, y= log10((HyPe_count/GeneModels_count)), color = Phylum)) +
  # ggalt::geom_encircle(data = MergedData %>% filter(Taxa == "Prokaryote", HyPe == "Production" | HyPe == "Scavenging"), alpha = 0.1, aes(x=log_Radius_um, y= log10((HyPe_count/GeneModels_count)), color = Phylum), expand = 0, s_shape = 0.9) +
  facet_grid(cols = vars(HyPe)) + #, rows = vars(Includes)
  scale_colour_manual(name = "Phylum", values =TaxaColors) +
...
ggsave(paste("FigureS11",".png",sep = ""), plot = HyPeFlagRiboPlot, device = NULL, path = Figures,scale = 1, height=7, width= 10, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("Figure6",".png",sep = ""), plot = NitOxRadPlots, device = NULL, path = Figures,scale = 1, height=7, width= 10, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("Figure7",".png",sep = ""), plot = NitOxPennateCentricPlot, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
...
NormalizeFlagellaPlot <- MergedData %>%
  filter(Taxa != "Prokaryote",
         #NitOx == "Production" | NitOx == "Scavenging",
         GeneModels_count != 0,
         RibosomeCount != 0,
         !is.na(Flagella),) %>%
  pivot_longer(c(GeneModels_count, RibosomeCount), names_to = "Normalizer", values_to = "Normalize_count") %>%
  select(FileName, Flagella, Phylum, Normalizer, Normalize_count) %>%
  unique() %>%
  ggplot(aes(x = as.factor(Flagella), y = Normalize_count)) +
  geom_boxplot(notch = TRUE, outlier.shape = NA) +
  geom_jitter(aes(colour = Phylum),width = 0.2, stat = "unique") +
  # coord_cartesian(ylim = c(ROSBoxPlotMin, ROSBoxPlotMax)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("") +
  facet_wrap(vars(Normalizer), 
             scales = "free", 
             labeller = as_labeller(c(GeneModels_count='Total Number of Genes', 
                                      RibosomeCount='Total Number of Ribosomal Genes'))) +
  stat_compare_means(comparisons = MyFlagellaComparisons, method = "wilcox.test") +
  # ggforce::facet_zoom(ylim = c(0, ROSBoxPlotMax)) +
  scale_x_discrete(labels = c("no" = "No Flagella", "yes" = "Flagella")) +
  scale_y_continuous(labels = scales::scientific) +
...
NormalizeFlagellaPlot
```

```{r}
NormalizeColonyPlot <- MergedData %>%
  filter(Taxa != "Prokaryote",
         #NitOx == "Production" | NitOx == "Scavenging",
          GeneModels_count != 0,
         RibosomeCount != 0,
         !is.na(ColonySpecies),) %>%
  pivot_longer(c(GeneModels_count, RibosomeCount), names_to = "Normalizer", values_to = "Normalize_count") %>%
  select(FileName, ColonySpecies, Phylum, Normalizer, Normalize_count) %>%
  unique() %>%
  ggplot(aes(x = as.factor(ColonySpecies), y = Normalize_count)) +
  geom_boxplot(notch = TRUE, outlier.shape = NA) +
  geom_jitter(aes(colour = Phylum),width = 0.2, stat = "unique") +
  # coord_cartesian(ylim = c(ROSBoxPlotMin, ROSBoxPlotMax)) +
  theme_bw() +
...
```{r}
TotalGenePennateCentricPlot <- MergedData %>%
  filter(Taxa == "Diatom",
         #NitOx == "Production" | NitOx == "Scavenging",
          GeneModels_count != 0,
         RibosomeCount != 0,
         !is.na(PennateCentric),) %>%
  pivot_longer(c(GeneModels_count, RibosomeCount), names_to = "Normalizer", values_to = "Normalize_count") %>%
  select(FileName, PennateCentric, Normalize_count, Normalizer, Phylum, Genus) %>%
  unique() %>%
  ggplot(aes(x = PennateCentric, y = Normalize_count)) +
  geom_boxplot(notch = TRUE, outlier.shape = NA) +
  geom_jitter(aes(colour = Genus),width = 0.2, stat = "unique") +
  # coord_cartesian(ylim = c(ROSBoxPlotMin, ROSBoxPlotMax)) +
  theme_bw() +
...
ggsave(paste("ManualAutoCompare",".png",sep = ""), plot = ManualAutoCompare, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("PoissonDist",".png",sep = ""), plot = PoissonDist, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)

ggsave(paste("FigureS5",".png",sep = ""), plot = PoissonDist, device = NULL, path = Figures,scale = 1, height=7, width= 7, units = c("in"),dpi = 300, limitsize = TRUE)
```
...
ggsave(paste("OchrophyteRadPlot",".png",sep = ""), plot = OchrophyteRadPlot, device = NULL, path = Figures,scale = 1, height=6, width= 10, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("FigureS9",".png",sep = ""), plot = EukHyPeDotPlot, device = NULL, path = Figures,scale = 1, height=20, width= 12, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("FigureS10",".png",sep = ""), plot = ProkHyPeDotPlot, device = NULL, path = Figures,scale = 1, height=10, width= 10, units = c("in"),dpi = 300, limitsize = TRUE)
...
ggsave(paste("FigureS12",".png",sep = ""), plot = EukNitOxDotPlot, device = NULL, path = Figures,scale = 1, height=20, width= 10, units = c("in"),dpi = 300, limitsize = TRUE)
