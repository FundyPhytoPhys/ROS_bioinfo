...
if [[ $isDNA == 0 ]]
    ...
    then
        ...
        python3.9 ~/Data/eggnog-mapper-2.0.6/emapper.py -i ${input} --output output/eggNOG/${output} -m ${m} --cpu ${cpu} --tax_scope ${tax_scope} --go_evidence ${go_evidence} --target_orthologs ${target_orthologs} --seed_ortholog_evalue ${seed_ortholog_evalue} --seed_ortholog_score ${seed_ortholog_score} --query_cover ${query_cover} --subject_cover ${subject_cover} 

    else
	# else if DNA, translate and run eggNOG
        echo "eggNOG with DNA..."
        python3.9 ~/Data/eggnog-mapper-2.0.6/emapper.py --translate -i ${input} --itype CDS --output output/eggNOG/${output} -m ${m} --cpu ${cpu} --tax_scope ${tax_scope} --go_evidence ${go_evidence} --target_orthologs ${target_orthologs} --seed_ortholog_evalue ${seed_ortholog_evalue} --seed_ortholog_score ${seed_ortholog_score} --query_cover ${query_cover} --subject_cover ${subject_cover}
fi
echo --------------------------